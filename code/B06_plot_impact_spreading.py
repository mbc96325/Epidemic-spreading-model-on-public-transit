...
def plot_impact(save_fig, scale_or_not):
    if scale_or_not:
        results = pd.read_csv('../data/impact_of_spreading_scale.csv')
    else:
    ...
    if save_fig == 1:
        if scale_or_not:
            plt.savefig('img/impact_of_spreading_scale.eps', dpi=200)
        else:
            plt.savefig('img/impact_of_spreading.eps', dpi=200)
    else:
        plt.show()
    a=1
...
if __name__ == '__main__':
    ...
    scale_or_not = True # True = use scaled data
