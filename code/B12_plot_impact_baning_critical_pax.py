...
def plot_impact(save_fig, scale_or_not):
    ...
    if scale_or_not:
        k_core_results = pd.read_csv('../data/impact_of_k_core_scale.csv')
    else:
    ...
    if scale_or_not:
        y_ticks = list(np.arange(2.1, 2.8, 0.1))
    else:
    ...
    if save_fig == 1:
        if scale_or_not:
            plt.savefig('img/impact_k_core_scale.eps', dpi=200)
        else:
            plt.savefig('img/impact_k_core.eps', dpi=200)
    else:
        plt.show()
