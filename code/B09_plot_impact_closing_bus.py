...
def plot_impact(save_fig, scale_or_not):
    if scale_or_not:
        ...
        results2 = pd.read_csv('../data/impact_of_close_bus_random_scale.csv')
        results3 = pd.read_csv('../data/impact_of_close_bus_low_high_scale.csv')
    else:
        results = pd.read_csv('../data/impact_of_close_bus_high_low.csv')
    ...
    if scale_or_not:
        y_ticks = list(np.arange(2.10, 2.70, 0.1))
    else:
    ...
    if save_fig == 1:
        if scale_or_not:
            plt.savefig('img/impact_of_close_bus_scale.png', dpi=200)
        else:
